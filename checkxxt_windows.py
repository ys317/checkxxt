...
WECOM_WEBHOOK_URL = ''   #例如https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=70e679aa-08ce-47l8-8549-2da9ec312120
CHAOXING_PHONE = ''      #账号
CHAOXING_PWD = ''        #密码
...
def check_chaoxing_homework():
    ...
    with sync_playwright() as p:
        ...
        print("登录超星...")
        page.goto("https://notice.chaoxing.com/pc/notice/myNotice")
        try:
        ...
        # Windows 登录后可能被重定向到 HTTP 版本，强制跳转 HTTPS 新版收件箱
        print("加载收件箱...")
        page.goto("https://notice.chaoxing.com/res/pc/mobileHtml/html/inbox.html")
        time.sleep(3)
        page.wait_for_selector('ul.item', timeout=15000)
        time.sleep(2)
        ...
        for idx, item_data in dl_items + hw_items[:10]:
            try:
                ...
                # 等待跳转到详情页
                try:
                    page.wait_for_url('**/noticeDetail**', timeout=10000)
                except:
                    # 如果没跳转，直接尝试从当前页面内容中找 noticeId
                    print("    点击后未跳转，尝试从页面提取...")
                    time.sleep(1)
